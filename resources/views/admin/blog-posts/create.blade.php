...
<div class="max-w-4xl">
    <form action="{{ route('admin.blog-posts.store') }}" method="POST" class="bg-white rounded-lg shadow-md p-6">
        @csrf
        ...
        <div class="mb-6">
            ...
            <textarea id="excerpt" 
                      ...
                      rows="3" 
                      required
                      class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent @error('excerpt') border-red-500 @enderror">{{ old('excerpt') }}</textarea>
            @error('excerpt')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
